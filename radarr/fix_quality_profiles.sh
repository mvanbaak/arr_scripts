...
# Script to fix movies with a physical release date that carry the wrong
# quality profile (e.g. [SQP] SQP-3 (Audio)) by switching them to the
# correct Remux-only profile (e.g. SQP-3-RemuxOnly).
#
# Requirements:
...
#
# Version 0.1.0 (Released 2026-08-08)
...
debug_log "=== Quality Profile Fix ==="
debug_log "Wrong profile: ${WRONG_PROFILE_NAME}"
debug_log "Correct profile: ${CORRECT_PROFILE_NAME}"
...
debug_log "Resolving wrong profile ID"
WRONG_PROFILE_ID=$(_resolve_profile_id "${WRONG_PROFILE_NAME}")
_resolve_rc=$?
if [ "${_resolve_rc}" -ne 0 ]
then
    exit 1
fi
debug_log "Wrong profile: ${WRONG_PROFILE_NAME} (id: ${WRONG_PROFILE_ID})"

debug_log "Resolving correct profile ID"
CORRECT_PROFILE_ID=$(_resolve_profile_id "${CORRECT_PROFILE_NAME}")
_resolve_rc=$?
if [ "${_resolve_rc}" -ne 0 ]
then
    exit 1
fi
debug_log "Correct profile: ${CORRECT_PROFILE_NAME} (id: ${CORRECT_PROFILE_ID})"

if [ "${WRONG_PROFILE_ID}" = "${CORRECT_PROFILE_ID}" ]
then
    echo "ERROR: Wrong and correct profiles are the same (id: ${WRONG_PROFILE_ID})" >&2
    echo "ERROR: Nothing to fix. Exiting." >&2
    exit 1
fi
...
_CANDIDATES=$(printf '%s' "${_ALL_MOVIES}" | jq \
    --arg wrong_id "${WRONG_PROFILE_ID}" \
    '
[.[] | select(
    .physicalRelease != null
    and .qualityProfileId == ($wrong_id | tonumber)
...
then
    ...
    then
        ...
        printf '%s' "${_CANDIDATES}" | jq \
            --arg wrong_name "${WRONG_PROFILE_NAME}" \
            --arg wrong_id "${WRONG_PROFILE_ID}" \
            --arg correct_name "${CORRECT_PROFILE_NAME}" \
            --arg correct_id "${CORRECT_PROFILE_ID}" \
            ...
            '
{
    wrong_profile: {id: ($wrong_id | tonumber), name: $wrong_name},
    correct_profile: {id: ($correct_id | tonumber), name: $correct_name},
    candidates: .,
...
then
    ...
    then
        echo
        echo "Quality Profile Fix"
        echo "==================="
        echo
        echo "Wrong profile: ${WRONG_PROFILE_NAME} (id: ${WRONG_PROFILE_ID})"
        echo "Correct profile: ${CORRECT_PROFILE_NAME} (id: ${CORRECT_PROFILE_ID})"
        echo
    fi
    ...
    then
        ...
        while IFS='|' read -r _title _phys_release _profile_id; do
            printf '%-50s %-12s %-20s  -> %s\n' "${_title}" "${_phys_release}" "${WRONG_PROFILE_NAME}" "${CORRECT_PROFILE_NAME}"
        done
        echo
    fi
...
do
    ...
    _payload=$(printf '{"movieIds": [%s], "qualityProfileId": %s}' "${_movie_id}" "${CORRECT_PROFILE_ID}")

    debug_log "Switching movie ${_movie_id} to profile ${CORRECT_PROFILE_ID}"
...
then
    echo "APPLY: Switched ${_SWITCHED_COUNT} movies to ${CORRECT_PROFILE_NAME}"
